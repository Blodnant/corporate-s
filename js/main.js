...
(function(html) {
  ...
  // Create a global array to store Masonry instances.
  window.msnryInstances = [];

  // Query for all containers with class "bricks"
  const containerBricks = document.querySelectorAll('.bricks');
  if (!containerBricks.length) return;

  // Loop through each container and initialize Masonry
  containerBricks.forEach(function(container) {
    imagesLoaded(container, function() {
      ...
      // Save the instance for later use (e.g., refreshing the layout)
      window.msnryInstances.push(msnry);
    });
  });
}; // end ssMasonry
